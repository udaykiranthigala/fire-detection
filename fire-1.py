...
vid = cv2.VideoCapture("candle.mp4") # To start camera this command is used "0" for laptop inbuilt camera and "1" for USB attahed camera
...
def play_alarm_sound_function(): # defined function to play alarm post fire detection using threading
    playsound.playsound('alarm.wav',True) # to play alarm # mp3 audio file is also provided with the code.
...
def send_mail_function(): # defined function to send mail post fire detection using threading
    ...
    recipientmail = "thigalaudaykiran258@gmail.com" # recipients mail
    ...
    try:
        ...
        server.ehlo()
        server.starttls()
        server.login("thigalaudaykiran258@gmail.com", 'Rru$2022') # Senders mail ID and password
        server.sendmail('thigalaudaykiran258@gmail.com', recipientmail, "Warning fire accident has been reported") # recipients mail with mail message
